...
allprojects {
  ...
  val spotlessFormatters: SpotlessExtension.() -> Unit = {
    ...
    kotlinGradle {
      ...
      // 1. The first argument (`rootProject.file("spotless/spotless.kt")`) points to the template
      // file containing the license text.
      // 2. The second argument is a regular expression that tells Spotless where the license should
      // end.
      //    It looks for the first line that starts with `import`, `plugins`, `buildscript`, etc.,
...
subprojects {
  ...
  configure<DetektExtension> {
    ...
    config.from(rootProject.file("config/detekt/detekt.yml")) // Use a custom Detekt config file.
